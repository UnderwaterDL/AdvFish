...
from adv_attacks import AdaFGSM

os.environ['CUDA_VISIBLE_DEVICES'] = '0'
# K.set_image_data_format('channels_first')

def train(dataset='Fish4Knowledge', cnn='resnet50', batch_size=32, epochs=50, image_size=32, eps=0.01):
    """
    Train one checkpoint with data augmentation: random padding+cropping and horizontal flip
    :param args: 
    :return:
    """
    print('Dataset: %s, CNN: %s, loss: adv, epsilon: %.3f batch: %s, epochs: %s'
    ...
    for ep in range(epochs-existing_ep):
        ...
        # save training log
        np.save(os.path.join(log_path, 'train_log_%s_%s_%.4f.npy' % (cnn, 'adv', eps)), log)
        ...
        if ((ep+existing_ep+1)%5 == 0) or (ep == (epochs - existing_ep - 1)):
            model_file = 'models/%s_%s_%s_%.4f_%s.h5' % (dataset, cnn, 'adv', eps, ep + existing_ep)
            model.save_weights(model_file)

def main(args):
    train(args.dataset, args.model, args.batch_size, args.epochs, args.image_size, args.epsilon)
...
if __name__ == "__main__":
    ...
    args = parser.parse_args(['-d', 'Fish4Knowledge', '-m', 'ResNet50',
                              '-e', '50', '-b', '32', '-s', '224', '-eps', '0.005'])
    main(args)


    # args = parser.parse_args(['-d', 'QUTFish', '-m', 'EfficientNetB3', '-e', '50',
    #                           '-b', '32', '-s', '224', '-eps', '0.005'])
    # main(args)

    # args = parser.parse_args(['-d', 'WildFish', '-m', 'ResNet50',
    #                           '-e', '50', '-b', '32', '-s', '224', '-eps', '0.005'])
    # main(args)
