...
from amsoftmax import AMSoftmax, amsoftmax_loss
...
def train(dataset='Fish4Knowledge', cnn='resnet50', batch_size=32, epochs=50, image_size=32):
    ...
    :return:
    """
    print('Dataset: %s, CNN: %s, loss: am-softmax, batch: %s, epochs: %s'
          % (dataset, cnn, batch_size, epochs))
    ...
    x = AMSoftmax(num_classes)(features)
    ...
    model_prefix = '%s_%s_%s_' % (dataset, cnn, 'mas')
    ...
    #
    #     log = np.load(os.path.join(log_path, 'train_log_%s_%s.npy' % (cnn, 'ams')))
    #
    ...
    for ep in range(epochs-existing_ep):
        ...
        for stp in pbar:
            ...
            pbar.set_postfix(loss='%.4f' % train_loss, acc='%.4f' % train_acc)
        ...
        # save training log
        np.save(os.path.join(log_path, 'train_log_%s_%s.npy' % (cnn, 'ams')), log)
        ...
        if ((ep+existing_ep+1)%5 == 0) or (ep == (epochs - existing_ep - 1)):
            model_file = 'models/%s_%s_%s_%s.h5' % (dataset, cnn, 'ams', ep+existing_ep)
