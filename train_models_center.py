...
from center_loss import CenterLossLayer, zero_loss

os.environ['CUDA_VISIBLE_DEVICES'] = '0'
# K.set_image_data_format('channels_first')

def train(dataset='Fish4Knowledge', cnn='resnet50', batch_size=32, epochs=50, image_size=32):
    """
    Train one checkpoint with data augmentation: random padding+cropping and horizontal flip
    :param args:
    :return:
    """
    print('Dataset: %s, CNN: %s, loss: center, batch: %s, epochs: %s' % (dataset, cnn, batch_size, epochs))
    ...
    num_features = np.prod(base_model.output_shape[1:])

    x = Dense(num_classes, activation='softmax', name='main_out')(features)
    model = Model(input=base_model.input, output=x, name='main_model')

    print('num_classes: ', num_classes, 'num_features: ', num_features)
    label_tensor = Input((num_classes,))
    center_layer = CenterLossLayer(alpha=0.5,
                                   num_classes=num_classes,
                                   num_features=num_features,
                                   name='center_layer')([features, label_tensor])

    final_model = Model(inputs=[model.input, label_tensor], outputs=[model.output, center_layer])
    # final_model.summary()
    ...
    model_prefix = '%s_%s_%s_' % (dataset, cnn, 'center')
    ...
    #
    #     log = np.load(os.path.join(log_path, 'train_log_%s_%s.npy' % (cnn, 'center')))
    #
    ...
    for ep in range(epochs - existing_ep):
        ...
        K.set_value(final_model.optimizer.lr, lr)
        ...
        for stp in pbar:
            b, l = get_batch()
            l_dumy = np.zeros((l.shape[0], 1))
            # print(final_model.metrics_names) #['loss', 'softmax_loss', 'center_layer_loss', 'softmax_accuracy']
            _, ce_loss, zr_loss, train_acc = final_model.train_on_batch(x=[b, l], y=[l, l_dumy])
            pbar.set_postfix(ce_loss='%.4f' % ce_loss, zr_loss='%.4f' % zr_loss, acc='%.4f' % train_acc)

        ## test acc and loss at each epoch
        reduced_model = Model(inputs=final_model.input[0], outputs=final_model.output[0])
        val_current_index = 0
        y_pred = []
        y_true = []
        while val_current_index + batch_size < num_images_val:
            b, l = get_val_batch()
            pred = reduced_model.predict(b)
        ...
        # save training log
        np.save(os.path.join(log_path, 'train_log_%s_%s.npy' % (cnn, 'center')), log)

        pbar.set_postfix(ce_loss='%.4f' % ce_loss, zr_loss='%.4f' % zr_loss, acc='%.4f' % train_acc,
                         val_acc='%.4f' % val_acc, val_loss='%.4f' % val_loss)
        print("Epoch %s - ce_loss: %.4f - zr_loss: %.4f - acc: %.4f - val_loss: %.4f - val_acc: %.4f"
              % (ep, ce_loss, zr_loss, train_acc, val_loss, val_acc))
        images, labels = shuffle(images, labels)
        if ((ep+existing_ep+1)%5 == 0) or (ep == (epochs - existing_ep - 1)):
            model_file = 'models/%s_%s_%s_%s.h5' % (dataset, cnn, 'center', ep+existing_ep)
            model.save_weights(model_file)

def main(args):
    train(args.dataset, args.model, args.batch_size, args.epochs, args.image_size)
...
if __name__ == "__main__":
    ...
    # args = parser.parse_args(['-d', 'Fish4Knowledge', '-m', 'EfficientNetB3',
    #                           '-e', '50', '-b', '32', '-s', '224'])
    # main(args)

    # args = parser.parse_args(['-d', 'QUTFish', '-m', 'EfficientNetB3',
    #                               '-e', '50', '-b', '32', '-s', '224'])
    # main(args)

    # args = parser.parse_args(['-d', 'WildFish', '-m', 'EfficientNetB3',
    #                           '-e', '50', '-b', '32', '-s', '224'])
    # main(args)
